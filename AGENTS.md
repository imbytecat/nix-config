...
- ponytail（`inputs.ponytail`，`flake = false`）两处装法不同：omp 直接吃上游自带的 pi extension —— `config.yml` 的 `extensions` 列上仓库根目录，loader 认 `package.json` 的 legacy `pi.extensions` 键，`before_agent_start` 每轮拼 ruleset，`/ponytail` 档位与 `/ponytail-*` 命令、`skills/` 都跟着来，不要退回手铺 skill link + 自造 frontmatter 的 rule；codex 用 `skills = "${inputs.ponytail}/skills"`，因为 HM 的 `programs.codex.plugins` 需要可写 plugin cache，`codex plugin add` 会往 store symlink 里写并直接 EROFS，marketplace 里只留一条 "not installed"。升级只用 `nix flake update ponytail`。
- caveman（`inputs.caveman`，ponytail 的另一半）没有 pi adapter：skills 只在 `codex.nix` 挂一次（`skillsIn` 把两个仓库的 `skills/` 合成 attrset），omp 靠自己的 codex provider 扫 `~/.codex/skills` 复用同一份 link —— 不要再给 omp 铺第二份，realpath 相同虽不报冲突但纯属重复；always-on 那层是把上游 `src/rules/caveman-activate.md` 包上 frontmatter 落成 omp 的 always-apply rule。
