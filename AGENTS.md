...
- **`flake.nix` 的 `nixConfig.extra-substituters` 给首次 bootstrap 用** —— 用户运行本仓任何 flake 命令时 nix 默认 ignoring untrusted；加 `--accept-flake-config` 或 `~/.config/nix/nix.conf` 写 `accept-flake-config = true`。**不要**在 `modules/shared/nix.nix` 里写 `accept-flake-config = true`，那是全局信任所有 flake 的行为变化，且首次 bootstrap 还没生效。
