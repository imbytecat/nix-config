...
- **Homebrew Gatekeeper / quarantine** — `caskArgs.no_quarantine` 已被 Homebrew 强制禁用（不再是 deprecation warning，是 hard error）。所有 cask 必须自带 Gatekeeper（签名 + 公证）；未公证的（例如某些国产软件）首次启动若被 Gatekeeper 拦下，用户手动 `xattr -d com.apple.quarantine /Applications/<App>.app` 处理，或在「系统设置 → 隐私与安全」点「仍要打开」。**不要**再把 `caskArgs.no_quarantine = true;` 加回来。
