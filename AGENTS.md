...
- **AI agent 工具走 `numtide/llm-agents.nix`** — `overlays/default.nix` 拼了 `inputs.llm-agents.overlays.default`，通过 `pkgs.llm-agents.<name>` 访问（`home/dev/ai.nix` 当前装 `claude-code` / `opencode` / `skills`）。**不要**让 `llm-agents` follows 本仓 `nixpkgs`，否则 `cache.numtide.com` 直接 miss（numtide CI 是用它自锁的 nixpkgs revision 构建的）。`home/dev/ai.nix` 还会写 `~/.claude/settings.json`（关 attribution / co-author、`opus[1m]`、跳危险模式提示），属于强声明，改 claude 行为要动这里。`omo` 是 fish 函数（`home/shell/fish.nix`），通过 `OPENCODE_CONFIG_DIR=$HOME/.config/opencode-profiles/omo opencode` 启动备用 profile，**不是** opencode plugin，也不在 `opencode.jsonc` 里登记。
...
- **注释最小化**：用户偏好"代码即文档"。只留**从 option/cask 名字推不出的 WHY**（如 FileVault 前置、`idleTime` 用 `-currentHost` 兜底、cyberduck 那个魔法数字），**不写**重述代码做什么的（如 cask 名后面跟"远程桌面"、`option = false` 后面跟"关闭 X"）。Decision rationale 留在 commit message 里，不灌进文件。
