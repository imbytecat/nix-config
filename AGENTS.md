...
- Agent 配置的落法：`codex` 走 HM 的 `programs.codex.settings`；共享 skills 由 `home/dev/agents/skills.nix` 逐项 link 到官方用户级目录 `~/.agents/skills`，父目录保持可写，Nix 只拥有声明的名字，`$CODEX_HOME/skills` 留给 CLI 的可变安装；`omp` 没有能用的模块（`programs.pi-coding-agent` 仍指着旧 `~/.pi/agent` 的 `settings.json`/`models.json`），继续用裸 `home.file` 写 `~/.omp/agent/*.yml`。
- ponytail（`inputs.ponytail`，`flake = false`）两层接法不同：omp 直接吃上游自带的 pi extension —— `config.yml` 的 `extensions` 列上仓库根目录，loader 认 `package.json` 的 legacy `pi.extensions` 键，`before_agent_start` 每轮拼 ruleset，`/ponytail` 档位与 `/ponytail-*` 命令都跟着来；通用 skills 则由 `skills.nix` 只铺一份到 `~/.agents/skills`，Codex/Orca/omp 共用。不要改用 HM `programs.codex.plugins`：`codex plugin add` 会往 store symlink 里写并直接 EROFS。升级只用 `nix flake update ponytail`。
- caveman（`inputs.caveman`，ponytail 的另一半）没有 pi adapter：skills 同样只由 `skills.nix` 铺到 `~/.agents/skills`，omp 的 agents provider 会扫该目录，不要再铺第二份；always-on 那层是把上游 `src/rules/caveman-activate.md` 包上 frontmatter 落成 omp 的 always-apply rule。`inputs.agent-browser` 只 pin Vercel 上游的 discovery stub，详细 workflow 由 `llm-agents` 安装的 `agent-browser` CLI 按自身版本提供。
