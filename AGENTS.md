...
Note: `eval`/`switch`/`build`/`dry`/`deploy`/`deploy-boot` 有 `[macos]`/`[linux]` 变体（justfile 按当前执行机平台自动选）；`boot`/`rollback` 仅 `[linux]`（NixOS-only，darwin 上无此 recipe）；`install` / `check` / `diff` / `lsp` 及 flake/nix 维护命令跨平台单一实现。
...
- **proto** — runtime/version management (`home/dev/languages.nix`)，替换了原来的 mise。home-manager **没有** `programs.proto` 模块，所以走「`pkgs.proto` 包 + fish 激活」：`programs.fish.interactiveShellInit` 里 `proto activate fish --no-shim | source`（cd/提示符变化触发 hook 自动切版本，等价 mise activate）。**设计是「nix 全局 + proto 项目级覆盖」**：`bun/go/nodejs/python3` 仍由 `home.packages` 装作全局默认；`--no-shim` 让 proto 不把 `~/.proto/shims` 塞进 PATH（否则一旦 `proto install` 某工具，其 shim 会 prepend 进 PATH 全局抢占 nix），于是没有 `.prototools` 的目录照常用 nix 全局版本，只有项目 pin 了才被 proto 覆盖。activate 默认 `upwards` 模式**不含**全局 `~/.proto/.prototools`（要 `--config-mode all` 才含），刚好保证 nix 是真·全局。版本配置走 `.prototools`（项目级）/`~/.proto/.prototools`（全局），由 proto 自己 imperative 管（`proto install` / `proto pin`），**不要**在 nix 里把 `~/.proto` 钉成只读 symlink，否则 `proto pin --to global` 会失败。
- **devenv** — CLI 在 `home/dev/devenv.nix`（`pkgs.devenv`；HM 无 `programs.devenv` 模块，只是装包）。binary cache `devenv.cachix.org` **只**进 `modules/shared/nix.nix` 的 `nix.settings`，**故意不进** `flake.nix` 的 `nixConfig`（bootstrap 块只服务本仓系统构建，与 devenv 无关）。per-project 的 `devenv.nix`/`devenv.yaml` 不归本仓管（本仓只做系统配置）。
...
- **Binary cache** — `modules/shared/nix.nix` 按命中率排序：`cache.nixos.org` → `nix-community` → `nixpkgs-unfree` → `cache.numtide.com`（llm-agents 产物）→ `devenv.cachix.org`（devenv 产物，见上 devenv 条）。`cache.garnix.io` 已于 2026-07 关停（garnix 服务并入 Shopify），不要再加回来。
...
- **HM `programs.*` 不要写 `enableFishIntegration = true;`** — HM 自 2025-02-07 起继承 `home.shell.enableShellIntegration`（默认 `true`），显式 `true` 是噪声。仅在主动关闭时写 `false`（如 `programs.zellij.enableFishIntegration = false`）。**别和 nix-darwin 的 `homebrew.enableFishIntegration` 混淆**：后者默认 `false`，必须显式 `true`（见 Gotchas），不要误删。
